...
try:
    ...
    print("ERROR: PRESTO Python module not found. Please install PRESTO.", file=sys.stderr)
    sys.exit(1)
...
class Candlist(object):
    ...
    def to_file(self, candfilenm=None):
        ...
        candfile.write("#" + "file:candnum".center(66) + "DM".center(9) +
                       "SNR".center(8) + "sigma".center(8) + "numharm".center(9) +
                       "ipow".center(9) + "cpow".center(9) +  "P(ms)".center(14) +
                       "r".center(12) + "z".center(8) + "w".center(8) + "numhits".center(9) + "\n")
    ...
    def to_cand_file(self, candfilenm):
        """Write Candlist to a candidate file format cand_num, f, fd, fdd, DM, sigma
            Input:
                candfilenm: Name of file to write to.
            Outputs:
                None
        """

        candfile = open(candfilenm, "w")
        candfile.write("# cand_num    f(Hz)      fd(Hz/s)    fdd(Hz/s^2)   DM(pc cm^-3)   sigma\n")
        for goodcand in self.cands:
            candfile.write("%8d %10.6f %10.6f %10.6f %14.2f %8.2f\n" %
                            (goodcand.candnum, goodcand.f, goodcand.fd,
                                goodcand.fdd, goodcand.DM, goodcand.sigma))
